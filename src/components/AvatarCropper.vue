<template>
  <el-dialog title="上传头像" :model-value="avatarDialogVisible" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false" destroy-on-close width="600px">
    <div class="avatar-container">
      <!-- 待上传图片 -->
      <div v-show="!options.img">
        <el-upload class="upload" ref="elUpload" action="#" :on-change="upload" accept="image/png, image/jpeg, image/jpg" :show-file-list="false" :auto-upload="false">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const upload = file => {
      ...
      if (!isIMAGE) {
        proxy.$message.warning('请选择 jpg、png 格式的图片')
        return false
      }
      if (!isLt5M) {
        proxy.$message.warning('图片大小不能超过 5MB')
        return false
      }
    ...
    const getCrop = () => {
      ...
      cropper.value.getCropBlob(data => {
        ...
        formData.append('file', data, fileName.value)
        //上传接口
        proxy.$http.post('/file/upload', formData).then(res => {
          emit('avatarSuccess', res)
        })
        closeDialog()
      })
    }
